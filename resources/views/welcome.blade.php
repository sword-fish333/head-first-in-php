...
<body
    ...
    style="background-image: url('{{asset('images/php_logo.png')}}'); background-size:60%; background-position: left top; background-repeat: no-repeat;">
...
<div
    ...
    <main class="flex max-w-[1/2] w-full flex-col-reverse lg:max-w-6xl lg:flex-row">
        <div
            ...
            <h1 class="font-medium text-xl">Concepte fundamentale în programare</h1>
            <h4 class="mb-3 text-[#706f6c]">Transpiră detaliile</h4>
            <p class="mb-4">Învață conceptele teoretice în PHP prin exemple practice. De la conceptele de bază până la cele
                mai abstracte.</p>
                ...
                >
                    <ul class="flex flex-col mb-4 lg:mb-6">
                        @foreach($tab['links'] as $linkIndex => $link)
                            <x-welcome_link :tab="$tab" :linkIndex="$linkIndex" :link="$link"/>

                        @endforeach
                        @isset($tab['sections'])
                                @foreach($tab['sections'] as $section_name=>$section)
                            <h5 class="mt-2 text-lg text-[#706f6c]">
                                {{$section['name']}}
                            </h5>
                                @foreach($section['links'] as $linkIndex => $link)
                                <x-welcome_link :tab="$section" :linkIndex="$linkIndex" :link="$link"/>
                                @endforeach
                                @endforeach
                            @endisset
                    </ul>
